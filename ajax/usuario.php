...
switch ($_GET["op"]) {
	...
	if (empty($idusuario)) {
		$rspta=$usuario->insertar($nombre,$tipo_documento,$num_documento,$direccion,$telefono,$email,$cargo,$login,$clavehash,$imagen,$_POST['permiso'],$_POST['categoria']);
		echo $rspta ? "Datos registrados correctamente" : "No se pudo registrar todos los datos del usuario";
	}else{
		$rspta=$usuario->editar($idusuario,$nombre,$tipo_documento,$num_documento,$direccion,$telefono,$email,$cargo,$login,$clavehash,$imagen,$_POST['permiso'],$_POST['categoria']);
...
//obtener permisos asigandos
$id=$_GET['id'];
$marcados=$categoria->listarmarcados($id);
$valores=array();

//almacenar permisos asigandos
while ($per=$marcados->fetch_object()) {
	array_push($valores, $per->idcategoria);
}

while ($reg = $rspta->fetch_object()) {
    $sw = in_array($reg->idcategoria, $valores) ? 'checked' : '';
    echo '<li><input type="checkbox" '.$sw.' name="categoria[]" value="'.$reg->idcategoria.'">'.$reg->nombre.'</li>';
}
